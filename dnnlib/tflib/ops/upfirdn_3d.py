...
def upfirdn_3d(x, k, upx=1, upy=1, upz=1 , downx=1, downy=1, downz=1, padx0=0, padx1=0, pady0=0, pady1=0, padz0=0, padz1=0, impl='cuda'):
...
def _upfirdn_3d_ref(x, k, upx=1, upy=1, upz=1 , downx=1, downy=1, downz=1, padx0=0, padx1=0, pady0=0, pady1=0, padz0=0, padz1=0):
    ...
    assert kernelD >= 1 and kernelH >= 1 and kernelW >= 1
    ...
    x = tf.reshape(x, [-1, inD, 1, inH, 1, inW, 1, minorDim])
    x = tf.pad(x, [[0, 0], [0, 0], [0, upz - 1], [0, 0], [0, upy - 1], [0, 0], [0, upx - 1], [0, 0]])
    ...
    # what is the correct filter shape? EXAM this
    w = tf.constant(k[::-1, ::-1, ::-1, np.newaxis, np.newaxis], dtype=x.dtype)
    x = tf.nn.conv2d(x, w, strides=[1,1,1,1,1], padding='VALID', data_format='NCDHW')
...
def upsample_3d(x, k=None, factor=2, gain=1, data_format='NCDHW', impl='cuda'):
...
def downsample_3d(x, k=None, factor=2, gain=1, data_format='NCDHW', impl='cuda'):
...
def upsample_conv_3d(x, w, k=None, factor=2, gain=1, data_format='NCDHW', impl='cuda'):
    ...
    convD = w.shape[0].value
    convH = w.shape[1].value
    convW = w.shape[2].value
    inC = _shape(w, 3)
    outC = _shape(w, 4)
    assert convW == convH
    ...
    if data_format == 'NCDHW':
        ...
        output_shape = [_shape(x, 0), outC, (_shape(x, 2) - 1) * factor + convD, (_shape(x, 3) - 1) * factor + convH, (_shape(x, 4) - 1) * factor + convW]
        num_groups = _shape(x, 1) // inC
    else:
        stride = [1, factor, factor, factor, 1]
        output_shape = [_shape(x, 0), (_shape(x, 1) - 1) * factor + convD, (_shape(x, 2) - 1) * factor + convH, (_shape(x, 3) - 1) * factor + convW, outC]
        num_groups = _shape(x, 3) // inC

    # Transpose weights.
    w = tf.reshape(w, [convD, convH, convW, inC, num_groups, -1])
    w = tf.transpose(w[::-1, ::-1, ::-1], [0, 1, 2, 5, 4, 3])
    w = tf.reshape(w, [convD, convH, convW, -1, num_groups * inC])
...
def conv_downsample_3d(x, w, k=None, factor=2, gain=1, data_format='NCHW', impl='cuda'):
    ...
    assert isinstance(factor, int) and factor >= 1
    w = tf.convert_to_tensor(w)
    convD, convH, convW, _inC, _outC = w.shape.as_list()
    ...
    return tf.nn.conv23(x, w, strides=s, padding='VALID', data_format=data_format)
...
def _simple_upfirdn_3d(x, k, up=1, down=1, pad0=0, pad1=0, data_format='NCDHW', impl='cuda'):
